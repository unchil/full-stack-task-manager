    ...
    routing{
        ...
        route("/nifs") {
            ...
            get("/seawaterinfo/{division}"){
                ...
                // 1. 캐시에서 데이터 조회
                val cachedData = cache_SeawaterInfo[key_SeawaterInfo]

                if (cachedData != null && now - cachedData.second < TimeUnit.SECONDS.toMillis(CACHE_EXPIRY_SECONDS)) {
                // 2. 캐시가 유효하면 즉시 응답
                    println("Serving from cache for ID: $division")
                    return@get call.respond(cachedData.first)
                }
            ...
            get ("/stat"){
                ...
                val now = System.currentTimeMillis()
                val cachedData = cache_SeaWaterInfoStatistics[key_SeaWaterInfoStatistics]
                if (cachedData != null && now - cachedData.second < TimeUnit.SECONDS.toMillis(CACHE_EXPIRY_SECONDS)) {
